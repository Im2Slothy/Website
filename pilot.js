document.addEventListener('DOMContentLoaded', () => {
  ...
  const maxSpeed = 2.5; // Significantly increased max speed
  const acceleration = 0.3; // Increased acceleration for responsiveness
  const deceleration = 0.01; // Reduced deceleration for smoother movement
  ...
  function shootBullet() {
    ...
    // Create two bullets for the dual weapons
    const offsetLeft = new THREE.Vector3(-1.2, 0, -0.5).applyQuaternion(playerShip.quaternion);
    const offsetRight = new THREE.Vector3(1.2, 0, -0.5).applyQuaternion(playerShip.quaternion);
    
    const bulletLeft = new THREE.Mesh(bulletGeometry, bulletMaterial);
    bulletLeft.position.copy(playerShip.position).add(offsetLeft);
    bulletLeft.velocity = direction.clone().multiplyScalar(bulletSpeed);
    bulletLeft.owner = peer.id;
    
    const bulletRight = new THREE.Mesh(bulletGeometry, bulletMaterial);
    bulletRight.position.copy(playerShip.position).add(offsetRight);
    bulletRight.velocity = direction.clone().multiplyScalar(bulletSpeed);
    bulletRight.owner = peer.id;
    ...
    const trailLeft = createBulletTrail();
    const trailRight = createBulletTrail();
    trailLeft.position.copy(bulletLeft.position);
    trailRight.position.copy(bulletRight.position);
    scene.add(trailLeft);
    scene.add(trailRight);
    
    // Attach trails to bullets
    bulletLeft.trail = trailLeft;
    bulletRight.trail = trailRight;
    ...
    connections.forEach(conn => {
      conn.send({
        type: 'bullet',
        x: bulletLeft.position.x,
        y: bulletLeft.position.y,
        z: bulletLeft.position.z,
        dirX: direction.x,
        dirY: direction.y,
        dirZ: direction.z
      });
      ...
      conn.send({
        type: 'bullet',
        x: bulletRight.position.x,
        y: bulletRight.position.y,
        z: bulletRight.position.z,
        dirX: direction.x,
        dirY: direction.y,
        dirZ: direction.z
      });
    ...
    aiShips.forEach((ai, index) => {
      ...
      if (ai.target) {
        ...
        const interceptPoint = ai.target.position.clone().add(
          targetVelocity.clone().multiplyScalar(distToTarget / bulletSpeed * 0.6) // Lead factor
        );
        ...
        ai.quaternion.slerp(targetQuat, 0.03); // Smooth rotation
        
        // Maintain optimal combat distance
        const optimalDistance = 50 + Math.random() * 20;
        
        // If too close, back up
        if (distToTarget < optimalDistance - 10) {
          ai.velocity.add(dirToTarget.clone().multiplyScalar(-0.1 * delta));
        } 
        // If too far, approach
        else if (distToTarget > optimalDistance + 10) {
          ai.velocity.add(dirToTarget.clone().multiplyScalar(0.1 * delta));
        }
        // At good distance, strafe (orbit) to avoid being a stationary target
        else {
          // Check if it's time for a new maneuver
          if (time > ai.maneuverTime) {
            ai.maneuverTime = time + 3000 + Math.random() * 2000;
            ...
            strafeDir.y = (Math.random() - 0.5) * 0.5;
            
            ai.maneuverDir = strafeDir;
          }
          
          // Apply the current maneuver
          ai.velocity.add(ai.maneuverDir.clone().multiplyScalar(0.1 * delta));
        }
        
        // Shoot at player with some randomization and cooldown
        if (time > ai.nextShotTime && distToTarget < 100) {
          ai.nextShotTime = time + 500 + Math.random() * 1500; // Adjust fire rate
        ...
        if (time > ai.maneuverTime) {
          ai.maneuverTime = time + 5000 + Math.random() * 3000;
...
ai.velocity.add(ai.maneuverDir.clone().multiplyScalar(0.05 * delta));
}

// Apply velocity with speed limit
const aiMaxSpeed = 1.5;
...
ai.velocity.multiplyScalar(0.98);
...
playerShip.rotation.y -= mouseX * mouseSensitivity * 30 * delta;
playerShip.rotation.x -= mouseY * mouseSensitivity * 20 * delta;

// Clamp pitch to prevent flipping
playerShip.rotation.x = Math.max(-Math.PI/3, Math.min(Math.PI/3, playerShip.rotation.x));

// Apply keyboard controls
const speedMultiplier = keys.boost ? 2.0 : 1.0; // Boost when shift is held

if (keys.forward) {
  const forward = new THREE.Vector3(0, 0, -1 * acceleration * speedMultiplier * delta);
...
if (keys.left) {
  const left = new THREE.Vector3(-1 * acceleration * 0.8 * speedMultiplier * delta, 0, 0);
...
const currentSpeed = playerShip.velocity.length();
if (currentSpeed > maxSpeed * speedMultiplier) {
  playerShip.velocity.normalize().multiplyScalar(maxSpeed * speedMultiplier);
}

// Apply natural deceleration
playerShip.velocity.multiplyScalar(1 - deceleration * delta);

// Update ship position
playerShip.position.add(playerShip.velocity.clone().multiplyScalar(delta));
...
bullets.forEach(bullet => {
  ...
  if (bullet.trail) {
    ...
    bullet.trail.lookAt(bullet.position.clone().add(bullet.velocity));
  }
});
...
connections.forEach(conn => {
  conn.send({
    ...
    rotationY: playerShip.rotation.y,
    rotationX: playerShip.rotation.x,
    rotationZ: playerShip.rotation.z
  });
});
}
